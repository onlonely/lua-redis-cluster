...
--debug object包装下
function _M.debug_object(self,...)
	local res,err=_do_cmd(self, "debug", "object",...)
	if res and res~=ngx.null then
		local rdata={}
				local tmp=string_split(res, " ")
				if #tmp>1 then
					local fd=rdata[tmp[1]]
					for j_index = 2, #tmp do
						local sp = string_split(tmp[j_index], ":", 2)
						rdata[sp[1]]=sp[2]
					end
				end
		return rdata
...
function _M.info(self,...)
	local res,err=_do_cmd(self, "info", ...)
	if res and res~=ngx.null then
		local rdata={}
		local fields=string_split(res,"# ")
		if #fields>1 then
			for i_index = 2, #fields do
				local tmp=string_split(fields[i_index], "\r\n")
				if #tmp>1 then
					rdata[tmp[1]]={}
					local fd=rdata[tmp[1]]
					for j_index = 2, #tmp do
